  ...
  const handleDelete = async () => {
    try {
      const response = await axios.delete(`http://localhost:3000/api/ocr/${deleteId}`);
  ...
  const loadHistory = async () => {
          try {
            const response = await axios.get('http://localhost:3000/api/ocr');
  ...
  const handleFileChange = (event) => {
    ...
    const handleSearch = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/api/ocr/users?name=${searchName}`);
